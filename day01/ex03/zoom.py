...
def zoom(scale_factor: float) -> np.ndarray | None:
    ...
    try:
        assert path and \
            isinstance(path, str), "Path must be a non-empty string."
        ...
        shape_info = cropped_image.shape[0], \
            cropped_image.shape[1], cropped_image.shape[2] \
            if len(cropped_image.shape) > 2 else 1
        print(f"New shape after slicing: \
              {shape_info} or {cropped_image.shape}")
